...
function genArm (which, name, elbowName)
    arm = gr.mesh('sphere', name)
    --torso:add_child(arm)
    arm:scale(1/0.5, 1/0, 1/0.5);
    arm:scale(0.2, 0.1, 0.1)
    arm:rotate('z', 50*which*-1);
    arm:translate(which*0.8, 0.0, 0.0)
    arm:set_material(red)
    
    elbow = gr.mesh('sphere', elbowName)
    arm:add_child(elbow)
    elbow:scale(1/0.5, 1/0, 1/0.5);
    elbow:scale(0.2, 0.1, 0.1)
    elbow:rotate('z', 50*which*-1);
    elbow:translate(which*0.8, 0.0, 0.0)
    elbow:set_material(red)

    return arm
end

leftArm = genArm(-1, 'leftArm', 'leftElbow')
rightArm = genArm(1, 'rightArm', 'rightElbow')
